...
float volume::calculateProb(int x, int y, int z)
{
  ...
  for(int d = 0; d < N_ALGS; d++)
  {
	...
	case A_BOUNDING_BOX:
	  ...
	  if ( acum > prob )
	  {
	    cerr << "this point should be eliminated for JAW[" << x << "," << y << "," << z \
		 << "]:" << this->matrix[z][y][x].i << " en:" << prob << endl;
	  }

	  acum = this->matrix[z][y][x].p[d] = 0.0;
...
bool volume::writeBox(std::ostream& output, ani::location& min, ani::location& max, float low, infoType info)
{
  ...
  cerr << "Box wrote " << count1 << " , " << count2 << " values"  << endl;
...
void volume::write(std::ostream& output, vformat fmt, int alg)
{
  ...
  switch(fmt)
  {
    case V_ANI_BINARY:
      // write only > MIN_PROB 
      cerr << "wrinting box for opacity using prob: "<< mprob << endl;
  ...
  case V_FAKE_BINARY:
      ...
      this->convolve(min, max, mprob, 4300);
